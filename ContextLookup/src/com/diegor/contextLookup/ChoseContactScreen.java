...
final class ChooseContactScreen extends MainScreen implements
	...
	private MenuItem lookupItem123people = new MenuItem(
		...
		public void run() {
			RemoteLookup lookup_email = new RemoteLookup();
			lookup_email.do123peopleLookup(user_input.getText());
			onClose();
		}
	...
	public void update(final net.rim.blackberry.api.mail.Message message) {
		...
		try {
			from_address = message.getFrom().getAddr();
		} catch (Exception ex) {
			from_address = "";
		}
		user_input = new EditField("Lookup: ", from_address);
		// TODO substitute with the pop-up text field like in the vanilla lookup
		// from the address book.
		user_input.setBorder(BorderFactory.createRoundedBorder(new XYEdges(10,
				10, 10, 10)));
		this.add(user_input);
		...
		String[/* field_type */][/* address_or_name */][/* members */] addressList = new String[recipientTypeListLength][2][/* different_for_al_fields */];
			...
			try {
				...
				if (numberOfRecipients > 0) {
					...
					for (int j = numberOfRecipients - 1; j >= 0; j--) {
						addressList[i][0][j] = message
								.getRecipients(recipientTypeIterator[i])[j]
								.getAddr();
						addressList[i][1][j] = message
								.getRecipients(recipientTypeIterator[i])[j]
								.getName();
					}
				}
		...
		for (int i = 0; i < recipientTypeListLength; i++) {
			if (addressList[i][0] instanceof String[]) { // not null
				if (addressList[i][0].length == 1) { // add a button instead of
					// a drop down list
					ButtonField bf = new ButtonField(addressList[i][0][0],
					...
					JustifiedHorizontalFieldManager hm = new JustifiedHorizontalFieldManager(
							lf, bf, true);
					// Had to create a custom Field Manager as the vanilla does
					// not support justified layout.
					bf.setChangeListener(this);
					this.add(hm);
				} else { // dropdown list
					ClickableObjectChoiceField ocf;
					this.add(ocf = new ClickableObjectChoiceField(
							fieldNameIterator[i], addressList[i][0]));
		...
		for (int i = 0; i < recipientTypeListLength; i++) {
			if (addressList[i][1] instanceof String[]) { // not null
				for (int k = (addressList[i][1].length - 1); k >= 0; k--) {
					// remove null members, implicitly using the reverse cycle
					// order
					if (!(addressList[i][1][k] instanceof String))
						Arrays.removeAt(addressList[i][1], k);
				}
				if (addressList[i][1].length == 1) {
					// add a button instead of a drop down list
					ButtonField bf = new ButtonField(addressList[i][1][0],
							ButtonField.CONSUME_CLICK);
					LabelField lf = new LabelField(fieldNameIterator[i]);
					JustifiedHorizontalFieldManager hm = new JustifiedHorizontalFieldManager(
							lf, bf, true);
					...
					ClickableObjectChoiceField ocf;
					this.add(ocf = new ClickableObjectChoiceField(
							fieldNameIterator[i], addressList[i][1]));
					ocf.setChangeListener(this);
				}
			}
		}
	...
	public void fieldChanged(Field field, int context) {
		...
		if (field instanceof ObjectChoiceField) {
			...
			// contacts
			try {
				user_input.setText((String) ((ObjectChoiceField) field)
